...
@section('content')
    <div class="card">
        <div class="card-header justify-content-between">
            <h3 class="card-title">Master Site</h3>
                    ...
                    <tbody>
                        @foreach ($data as $element)
                            <tr>
                                ...
                                <td>{{ $element->s_created_at }}</td>
                                <td>
                                    <div class="d-flex">
                                        <a class="btn btn-sm btn-warning p-2 me-1"
                                            href="{{ route('master_site_edit', ['id' => $element->s_id]) }}"><i
                                                class="ti-pencil"></i></a>
                                        <button type="button" class="btn btn-sm btn-danger p-2 ms-1"
                                            onclick="del({{ $element->s_id }})"><i class="ti-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            iziToast.question({
                ...
                buttons: [
                    ['<button style="background-color:red;"> Delete </button>', function(instance, toast) {
                        $.ajax({
                            url: baseUrl + '/master' + '/master_site/delete/' + id,
                            type: 'get',
                            success: function(data) {
                                if (data.status == 'sukses') {
                                    ...
                                    setTimeout(() => {
                                        window.location = ("{{ route('master_site') }}")
                                    }, 1250);
                                } else {
                                    iziToast.error({
                                        position: 'topRight',
