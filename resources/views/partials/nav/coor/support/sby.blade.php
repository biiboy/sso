...
<li class="nav-item mb-2 dropdown {{ request()->url() == route('assessment_all_approval') || request()->url() == route('assessment_all_approval_review_kpi_team_jakarta_helpdesk') ? 'active' : '' }}">
    <a class="nav-link dropdown-toggle {{ request()->url() == route('assessment_all_approval') || request()->url() == route('assessment_all_approval_review_kpi_team_surabaya_support') ? 'show' : '' }}" href="#navbar-extra" data-bs-toggle="dropdown" data-bs-auto-close="false" role="button" aria-expanded="false">
        <span class="nav-link-icon d-md-none d-lg-inline-block">
    ...
    <div class="dropdown-menu {{ request()->url() == route('assessment_all_approval') || request()->url() == route('assessment_all_approval_review_kpi_team_surabaya_support') ? 'show' : '' }}">
        <a class="dropdown-item {{ request()->url() == route('assessment_all_approval') ? 'active' : '' }}" href="{{ route('assessment_all_approval') }}">
            Approval
        </a>
        <a class="dropdown-item {{ request()->url() == route('assessment_all_approval_review_kpi_team_surabaya_support') ? 'active' : '' }}" href="{{ route('assessment_all_approval_review_kpi_team_surabaya_support') }}">
            Review
        </a>
    </div>
</li>
<li class="nav-item mb-2 dropdown {{ request()->is('all_report*') ? 'active' : '' }}">
    ...
    <div class="dropdown-menu {{ request()->is('all_report*') ? 'show' : '' }}">
        <a class="dropdown-item {{ request()->url() == route('all_report_support_coor_surabaya') ? 'active' : '' }}" href="{{ route('all_report_support_coor_surabaya') }}">
            KPI Chart
        </a>
        <div class="dropend">
            ...
            <div class="dropdown-menu {{ request()->is('all_report') ? 'show' : '' }}">
                <div class="dropend">
                    ...
                    <div class="dropdown-menu {{ request()->is('all_report') ? 'show' : '' }}">
                        <a href="{{ route('all_report', ['id' => 'Surabaya', 'flag' => '2']) }}" class="dropdown-item {{ request()->is('all_report') ? 'active' : '' }}">Surabaya</a>
                    </div>
                </div>
            </div>
        </div>
